...
public class BlockParser {
	...
	private void incorporateLine(String ln) {
		...
		while (true) {
			...
			if (indent >= CODE_INDENT) {
				// indented code
				if (this.tip.getType() != Node.Type.Paragraph && !blank) {
					offset += CODE_INDENT;
					allClosed = allClosed ||
		...
		if (!allClosed && !blank &&
				this.tip.getType() == Node.Type.Paragraph &&
			...
			boolean lastLineBlank = blank &&
					!(t == Node.Type.BlockQuote ||
							(t == Node.Type.CodeBlock && ((CodeBlock) container).isFenced()) ||
							(t == Node.Type.Item &&
	...
	private static boolean canContain(Node.Type parent_type, Node.Type child_type) {
		return (parent_type == Node.Type.Document ||
				parent_type == Node.Type.BlockQuote ||
				parent_type == Node.Type.Item || (parent_type == Node.Type.List && child_type == Node.Type.Item));
	}

	// Returns true if block type can accept lines of text.
	private static boolean acceptsLines(Node.Type block_type) {
		return block_type == Node.Type.Paragraph ||
				block_type == Node.Type.CodeBlock;
	}
